...
# Dec 12, 2023 - "Statistical tests to spot potential data outliers in R"
...
In today's post I will cover two hypothesis tests to spot several outliers simultaneously using R.

It is worth noting that the tests demonstrated below are not supposed to replace other outlier detection techniques.

These formal statistical tests must be used in combination with graphical and descriptive analyses and prior experience with the type of data and context in which the data was collected.
...
The two formal outlier detection tests presented below assume that our variable of interest follows a normal distribution.

Furthermore, these two tests assume the presence of more than one potential outlier. Therefore, Dixon's test and Grubb's test, which examine whether a *single* low or high value is an outlier at a time, will not be covered.
...
Instead, prior experience with the data's nature and collection process knowledgeable analyst's ability to spot unusual values and formal statistical tests should walk together.
