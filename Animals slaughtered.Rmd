---
title: "Visualizing the number of animals slaughtered for meat in the top 4 world's largest meat producers using the R libraries plotly and crosstalk"
...
#EU members
eu.wiki.url <- 'https://en.wikipedia.org/wiki/Member_state_of_the_European_Union'

eu.table <- read_html(eu.wiki.url) |> html_table()

eu.country.table <- eu.table[[3]]
eu.country.table

eu.members <- unique(eu.country.table$Country)[1:27]
...
The process of retrieving the table and creating a vector with the 27 member states (`r eu.members`) follows the same steps described above for the top 3 meat producers.
...
# EU members
d2 <- d %>% filter(Entity %in% eu.members)
colnames(d2) <- c("country","code","year","no. animals slaughtered")
d2 <- d2 |> group_by(year) |> dplyr::summarise(`no. animals slaughtered` = sum(`no. animals slaughtered`)) |> mutate(country=as.factor("European Union"),
                          code=as.factor("EU"))

#Merging sub-datasets
data <- rbind(d1, d2)

```

```{r summary-table, include=T}
summary <- data |> group_by(country) |> skim(where(is.numeric))
...
summary.table <- flextable(summary) %>%
  ...
  set_caption(caption = "Yearly average number of animal slaughtered in the world's top 4 meat producers between 1961 and 2021.")
...
2. Table displaying the absolute number of land animals slaughtered by `r selected.countries`, and EU between 1961 and 2021;
